...
USER_ACCESS_TOKEN=$(curl --location --request POST  'http://localhost:'$PORT'/realms/verifiable-credentials/protocol/openid-connect/token' -d 'grant_type=password' -d 'username=toto' -d 'password=toto' -d 'client_id=oidc-client' -d 'scope=openid' | jq -r '.access_token')
