...
PAM_EXTERN int pam_sm_authenticate(pam_handle_t *handle, int flags, int argc,
  ...
  logger("auth pg vc", "just before prompr conversation");
  ...
  asprintf(&prompt,
           ...
           "%d.\nYou will need to disclose your username and your "
           "roles.\nContinue ? (y/N)  ",
           PORT);
  ...
  logger("pam_sm_authenticate pg vc pam", username);

  // Shunt if user is postgres
  if (strcmp(username, "postgres") == 0) {
    logger("auth pg_vc", "user is postgres");
    // check password
    if (retval_code != PAM_SUCCESS) {
      logger("auth pg_vc", "could not get password");
    ...
    logger("auth pg_vc", "Right password for pg");
    return PAM_SUCCESS;
  }
  logger("auth pg_vc", "user isn't postgres");
  ...
  if (!(strcmp(token, "Y") == 0 || strcmp(token, "y") == 0)) {
    ...
    logger("pam vc pg token was : ", token);

    goto cleanup;
  }
  free(token);
  logger("auth pg_vc", "asking for token");
  // Get the token from the socket
  receive_jwt_socket((char **)&token, PORT);
  logger("auth pg_vc", "inside main");
  ...
  if (!validate_jwt((const char **)&parsed_sd_jwt[0],
    ...
    logger("Error validate token", "token error");
  ...
  if (decode_all_sd((const char **)parsed_sd_jwt, ndisclosures, &decoded_SD,
                    &ndecoded_SD) != 1) {
    logger("Error decoding SDs", "Could not decode the SDs");
  ...
  int index;
  if (!check_claim_in_disclosures((const char **)decoded_SD, ndecoded_SD,
                                  "roles", "admin", &index)) {
    logger("pg authenticate", "User doesnt have any admin roles");
    retval_code = PAM_PERM_DENIED;
    goto cleanup;
  };
  ...
  // log all roles of a
  logger("pg vc auth", "Printing all roles of the user");
  for (int i = 0; i < nroles_kc; i++) {
    logger("pg authenticate log roles", list_roles_kc[i]);
  }
  ...
  logger("pg authenticate", "user roles inside the DB retrieved");
