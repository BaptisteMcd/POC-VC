...
__attribute__((constructor)) void init(void) {
  ...
  if (pConfigFile == NULL) {
    ...
    logger("test",
           "Erreur lors de l'ouverture du fichier de configuration kc_auth");
    return;
  }
...
bool authentification_utilisateur(const char *user, const char *pass,
                                  char **p_access_token, char **p_refresh_token,
                                  char **p_id_token) {
  CURL *curl;
  CURLcode res;
  curl = curl_easy_init();
  ...
  if (curl) {
    char *url;
    asprintf(&url, "http://%s:%s/realms/%s/protocol/openid-connect/token",
             KEYCLOAK_IP, KEYCLOAK_PORT, REALM_NAME);
    ...
    char *data;
    asprintf(&data,
             "client_id=%s&client_secret=%s&username=%s&password=%s&grant_type="
  ...
  if (res != CURLE_OK) {
    ...
    if (response_code == 200) { // Succeeded
      jsmn_parser p;
      jsmntok_t t[128];
      jsmn_init(&p);
      int r =
          jsmn_parse(&p, chunk.memory, chunk.size, t, sizeof(t) / sizeof(t[0]));
      if (r < 0) { // Parsing failed
        printf(
            "Failed to parse JSON performing request for Client token : %d\n",
            r);
        printf("%s\n", chunk.memory);
        ...
        for (int i = 1; i < r; i++) {
          if (jsoneq(chunk.memory, &t[i], "access_token") == 0) {
            ...
            asprintf(p_refresh_token, "%.*s", t[i + 1].end - t[i + 1].start,
                     chunk.memory + t[i + 1].start);
            i += 1;
          }
        }
      ...
      logger("Authentification utilisateur",
             "Failed : got wrong response code");
...
const bool verif_existance_utilisateur(const char *nom_utilisateur,
  ...
  if (curl) {
    ...
    asprintf(&request_url,
             "http://%s:%s/admin/realms/%s/users?exact=true&username=%s",
             KEYCLOAK_IP, KEYCLOAK_PORT, REALM_NAME, nom_utilisateur);
  ...
  if (res != CURLE_OK) { // Bad response
    fprintf(stderr, "curl_easy_perform() failed: %s\n",
            curl_easy_strerror(res));
    printf(
        "Failed to perform request checking user existance, got code : %lu \n",
        response_code);
    logger("Existance utilisateur", "curl_easy_perform() failed");
    return false;
  } else { // Good response but it doesn't mean the user is found
    printf("Response code : %lu \n", response_code);
    if (response_code == 200) { // Good response, parsing ...
      ...
      if (r < 0) {
        printf("Failed to parse JSON performing request to check user "
               "existance : %d\n",
               r);
        logger("Existance utilisateur", "Failed to parse JSON");
      } else { // Parsing JSON
        for (int i = 1; i < r; i++) {
          if (jsoneq(chunk.memory, &t[i], "username") == 0) { // Found username
        ...
        printf("%s\n", chunk.memory);
        if (result_username == NULL) { // No username found
          logger("Existance utilisateur", "user not found");
        }
        if (strcmp(result_username, nom_utilisateur) == 0 &&
            t[0].size == 1) { // It's the right and only user
          found = true;
...
const bool deconnection(const char **p_access_token,
                        const char **p_refresh_token) {
  CURL *curl;
  CURLcode res;
  curl = curl_easy_init();
  if (curl) {
    curl_easy_setopt(curl, CURLOPT_CUSTOMREQUEST, "POST");
    char *url;
    asprintf(&url, "http://%s:%s/realms/%s/protocol/openid-connect/logout",
             KEYCLOAK_IP, KEYCLOAK_PORT, REALM_NAME);
    curl_easy_setopt(curl, CURLOPT_URL, url);
    curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION, 1L);
    curl_easy_setopt(curl, CURLOPT_DEFAULT_PROTOCOL, "https");
    ...
    char *data;
    asprintf(&data,
             "client_id=%s&client_secret=%s&Authorization=Bearer "
             "%s&refresh_token=%s",
             CLIENT_ID, CLIENT_SECRET, *p_access_token, *p_refresh_token);
    curl_easy_setopt(curl, CURLOPT_POSTFIELDS, data);
...
const bool getpubkey(char **p_public_key) {
  ...
  if (curl) {
    curl_easy_setopt(curl, CURLOPT_CUSTOMREQUEST, "GET");
    char *url;
    asprintf(&url, "http://%s:%s/realms/%s", KEYCLOAK_IP, KEYCLOAK_PORT,
             REALM_NAME);
    curl_easy_setopt(curl, CURLOPT_URL, url);
    curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION, 1L);
    curl_easy_setopt(curl, CURLOPT_DEFAULT_PROTOCOL, "https");
  ...
  if (res != CURLE_OK) { // Bad response
    ...
    if (r < 0) {
      printf("Failed to parse JSON performing request to check user existance "
             ": %d\n",
             r);
      logger("Existance utilisateur", "Failed to parse JSON");
    } else { // Parsing JSON
      for (int i = 1; i < r; i++) {
        if (jsoneq(chunk.memory, &t[i], "public_key") == 0) { // Found pubkey
          asprintf(p_public_key,
                   "-----BEGIN PUBLIC KEY-----\n%.*s\n-----END PUBLIC KEY-----",
...
const bool validate_token(const char **p_token, const char **p_public_key,
                          char **claim, char **p_username_in_token) {
...
const bool parse_role_claims(const char **p_claims, const char *origin,
  ...
  jsmn_init(&p);
  int r =
      jsmn_parse(&p, *p_claims, strlen(*p_claims), t, sizeof(t) / sizeof(t[0]));
  bool success = false;
  if (r < 0) {
    printf("Failed to parse JSON performing request to check user existance : "
           "%d\n",
...
bool getUserRoles(PGconn *conn, const char *username, char ***p_retVal,
  ...
  char *query;
  asprintf(&query,
           "SELECT rolname FROM pg_roles WHERE pg_has_role('%s',oid,'member');",
           username);
  ...
  *p_retVal = (char **)malloc(
      sizeof(char *) * (*nretVal)); // Allocate the array of pointers of chars
  for (int j = 0; j < *nretVal; j++) {
  }
...
bool InitSearchPath(
    PGconn *conn) { /* Initialise un search path sûr, pour qu'un utilisateur
